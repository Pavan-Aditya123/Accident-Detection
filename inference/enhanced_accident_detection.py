"""
Zero-DCE + YOLO26 Integrated Accident Detection Pipeline.
...
traffic surveillance. Zero-DCE enhances dark frames before YOLO26 processes them,
...
class VideoProcessor:
    ...
    Coordinates Zero-DCE enhancement and YOLO26 detection.
    ...
    def __init__(
        ...
        """
        print("=" * 70)
        print("  ZERO-DCE + YOLO26 INTEGRATED ACCIDENT DETECTION PIPELINE")
        print("=" * 70)
    ...
    def process_video(
        ...
        """
        Process a video file through the integrated pipeline.
        ...
        if output_video_path is None:
            output_dir = self.base_dir / "results" / "enhanced_detection"
            output_dir.mkdir(parents=True, exist_ok=True)
            # Extract input video name and create output name
            input_filename = Path(input_video_path).stem
            output_video_path = output_dir / f"{input_filename}_output.mp4"
        ...
        while True:
            ...
            # Draw detections on enhanced frame
            drawing_start = time.time()
            annotated_frame = self.detector.draw_detections(enhanced_frame, results)
            ...
            # Optional display
            if display_enhanced:
                cv2.imshow('Enhanced Accident Detection', annotated_frame)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Zero-DCE + YOLO26 Integrated Accident Detection Pipeline'
    )
    parser.add_argument(
        '--video',
        type=str,
    ...
    if not input_video.exists():
        ...
        print(f"[!] Usage: python inference/enhanced_accident_detection.py --video <video_filename.mp4>")
        return
